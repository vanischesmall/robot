            ...
            #     print("in", answer, time.time())
                ...
                if message[0] == "stop":
                    ...
                    try:
                        ...
                        if len(message)>=2:
                            l= message[1]
                        socket_inet.send_string("stop|"+l)
                        answ = socket_inet.recv_string()
                        print(answ)
                        ic.send_to(answer[0], answ)
                        flag_stopt_key = True
                ...
                if message[0] == "lock":
                    ...
                    # посылаем данные консоли
                    try:
                        socket_inet.send_string("lock|" + str(message[1])+"|"+str(message[2]))
                        answ = socket_inet.recv_string()
                        print(answ)
                        ic.send_to(answer[0], answ)
                    except:
                ...
                if message[0] == "drop":
                    ...
                    # посылаем данные консоли
                    try:
                        socket_inet.send_string("drop|" + str(message[1])+"|"+str(message[2]))
                        answ = socket_inet.recv_string()
                        print(answ)
                        ic.send_to(answer[0], answ)
                    except:
                ...
                if message[0] == "run":
                    ...
                    socket_inet.send_string("run|" + str(message[1])+"|"+l)
                    answ = socket_inet.recv_string()
...
async def main():
    global console_out, proc, master_ip, filename, flag_locked, lock_pause
    ...
    while True:
        try:
            ...
            if message[0].find("stop") >= 0:
                print("try stop", message)
                if flag_locked is not None:
                    if len(message)>=2:
                        if message[1]!=str(flag_locked):
                            print("|fail_stop|robot_locked|")
                            socket.send_string("|fail_stop|robot_locked|")
                            continue
                    else:
                        socket.send_string("|fail_stop|wrong_format")
                        continue
                ...
                try:
                    ...
                    pass
                continue
            elif message[0].find("lock") >= 0:
                # назначаем хозяина
                answer="|lock|False|-1"
                if flag_locked is None:
                    flag_locked=time.time()+float(message[2])

                    answer="|lock|True|" + str(flag_locked)+"|\n"
                    print("lock", answer)
                else:
                    if message[1]==str(flag_locked):
                        flag_locked = time.time() + float(message[2])
                        answer = "|lock|True|" + str(flag_locked) + "|\n"
                        print("lock again", answer)

                    else:
                        answer = "|lock|False|" + str(int(flag_locked-time.time()))+"|\n"
                        print("False lock", answer)
                ...
                if flag_locked is None:
                    ...
                    if message[1] == str(flag_locked):
                        flag_locked = None
                        answer = "|drop|True|\n"
                        print("drop again", answer)

                    else:
                        answer = "|drop|False|\n"
                        print("False drop", answer)
                ...
                if flag_locked is not None:
                    if len(message) >= 3:
                        if message[2]!=str(flag_locked):
                            socket.send_string("|fail_run|robot_locked|")

                            continue
                    else:
                        socket.send_string("|fail_run|wrong_format|")
                        continue
